...
StoryLoaderDialog::StoryLoaderDialog(int sceneColumnCount, int particleColumnCount, AppContext &appContext, QWidget *parent) : appContext{appContext}, QDialog{parent}
{
  ...
  connect(dimensionality2DButton, &QRadioButton::toggled,
          [=, this](bool isDimensionality2D)
          {
            if (isDimensionality2D)
            {
              scenePropertyGrid->setVisibleComponents({true, true, false});
              particlePropertyGrid->setVisibleComponents({true, true, false});
            }
            else
            {
              scenePropertyGrid->setVisibleComponents({true, true, true});
              particlePropertyGrid->setVisibleComponents({true, true, true});
            }
          });
